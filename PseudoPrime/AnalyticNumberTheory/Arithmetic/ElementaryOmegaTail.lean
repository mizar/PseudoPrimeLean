...
theorem hasDerivAt_elementaryA {t : ℝ} (ht : 0 < t) :
    HasDerivAt elementaryA (1 / t) t := by
...
theorem elementaryA_pos {t : ℝ} (ht : 11 ≤ t) :
    (0 : ℝ) < elementaryA t := by
  linarith only [elementaryA_ge_two ht]

/-- `h'(t) = log(2/a(t)) / (t·a(t)²) ≤ 0` for `t ≥ 11`. -/
theorem hasDerivAt_elementaryH {t : ℝ} (ht : 11 ≤ t) :
    ∃ d, HasDerivAt elementaryH d t ∧ d ≤ 0 := by
  have ht0 : (0 : ℝ) < t := by linarith only [ht]
  ...
  have hLeq :
    L = Real.log t + Real.log (elementaryA t) := by
    rw [hLdef, huval, Real.log_mul ht0.ne' hapos.ne']
  have haeq :
    elementaryA t + 1 = Real.log 2 + Real.log t := by
    unfold elementaryA
  ...
  have hnum_eq :
    ((elementaryA t + 1) /
          ...
          L * (1 / t)) /
        elementaryA t ^ 2 =
      Real.log (2 / elementaryA t) /
        (t * elementaryA t ^ 2) := by
  ...
  have hden : (0 : ℝ) ≤ t * elementaryA t ^ 2 := by
    positivity
  exact div_nonpos_iff.mpr (Or.inr ⟨hloghalf, hden⟩)

theorem elementaryH_antitoneOn :
    AntitoneOn elementaryH (Set.Ici (11 : ℝ)) := by
  have hD : Convex ℝ (Set.Ici (11 : ℝ)) := convex_Ici _
  have hcont :
    ContinuousOn elementaryH (Set.Ici (11 : ℝ)) :=
    fun t ht =>
    (hasDerivAt_elementaryH
        (Set.mem_Ici.mp ht)).choose_spec.1.continuousAt.continuousWithinAt
  have hdiff :
    DifferentiableOn ℝ elementaryH
      (interior (Set.Ici (11 : ℝ))) := by
    rw [interior_Ici]
    intro t ht
    exact
      (hasDerivAt_elementaryH
  ...
  obtain ⟨d, hd, hd_nonpos⟩ :=
    hasDerivAt_elementaryH ht.le
  rwa [hd.deriv]

theorem elementaryU_ge {t : ℝ} (ht : 11 ≤ t) :
    (22 : ℝ) ≤ elementaryU t := by
  unfold elementaryU
  have ha2 := elementaryA_ge_two ht
  nlinarith only [mul_le_mul ht ha2 (by norm_num only : (0 : ℝ) ≤ 2)
      (by linarith only [ht] : (0 : ℝ) ≤ t)]

theorem elementaryH_nonneg {t : ℝ} (ht : 11 ≤ t) :
    0 ≤ elementaryH t := by
  unfold elementaryH
  apply div_nonneg
  · exact
      Real.log_nonneg
        (by linarith only [elementaryU_ge ht])
  · linarith only [elementaryA_pos ht]

/-- `F` is antitone on `[11, ∞)`: a product of the two antitone nonnegative factors
`1 + 1/t` and `h(t)`. -/
theorem elementaryF_antitoneOn :
    AntitoneOn elementaryF (Set.Ici (11 : ℝ)) := by
  ...
  have hh1 :
    elementaryH t2 ≤
      elementaryH t1 :=
    elementaryH_antitoneOn h1 h2 h12
  have hh2nn : 0 ≤ elementaryH t2 :=
    elementaryH_nonneg h2
  ...
  calc
    (1 + 1 / t2) * elementaryH t2 ≤
        (1 + 1 / t1) * elementaryH t2 :=
      mul_le_mul_of_nonneg_right hc1 hh2nn
    _ ≤ (1 + 1 / t1) * elementaryH t1 :=
      mul_le_mul_of_nonneg_left hh1 hc1pos

-- The single numeric certificate closing the tail: `F(163) ≤ 7/5`.
set_option exponentiation.threshold 2000 in
theorem elementaryF_163_le :
    elementaryF 163 ≤ (7 / 5 : ℝ) := by
  have hx1 : |(1 - (326 : ℝ) / 2 ^ 9)| < 1 := by norm_num only [abs_of_nonneg]
  obtain ⟨hlo1, hhi1⟩ :=
    Analysis.log_bounds_of_taylor (N := 326) (by norm_num only) 9 30 rfl hx1
  ...
  have hA163eq :
    elementaryA 163 = Real.log 326 - 1 := by
    unfold elementaryA; norm_num only
  have hA_lo :
    (4786897379 / 1000000000 : ℝ) ≤
      elementaryA 163 := by
    rw [hA163eq]; linarith only [hlog326_lo]
  have hA_hi :
    elementaryA 163 ≤ (598362173 / 125000000 : ℝ) := by
    rw [hA163eq]; linarith only [hlog326_hi]
  have hA_pos : (0 : ℝ) < elementaryA 163 := by
    linarith only [hA_lo]
  have hU163eq :
    elementaryU 163 =
      163 * elementaryA 163 :=
  ...
  obtain ⟨_, hhi2⟩ :=
    Analysis.log_bounds_of_taylor_real (N := (780264273592 / 1000000000 : ℝ))
      (by norm_num only) 10 30 rfl hx2
  have hlogU_hi :
    Real.log (elementaryU 163) ≤
      (6659632677 / 1000000000 : ℝ) := by
  ...
  have hgoal_eq :
    elementaryF 163 =
      (164 / 163 * Real.log (elementaryU 163)) /
        elementaryA 163 := by
...
theorem log_ge_elementaryU_of_pow_mul_factorial_le {n m : ℕ} (hm1 : 1 ≤ m)
    (hn : 2 ^ m * Nat.factorial m ≤ n) :
    elementaryU (m : ℝ) ≤ Real.log n := by
  have hnpos : (0 : ℝ) < n := by
  ...
  have hUeq :
    elementaryU (m : ℝ) =
      (m : ℝ) * (Real.log 2 + Real.log m - 1) := by
    unfold elementaryU
      elementaryA
...
theorem distinctPrimeFactorCount_eq_primeFactors_card (n : ℕ) :
    distinctPrimeFactorCount n =
      n.primeFactors.card := by
  rw [distinctPrimeFactorCount,
    ArithmeticFunction.cardDistinctFactors_apply, ← List.card_toFinset, Nat.toFinset_factors]
...
theorem elementaryOmegaStatement_tail {n : ℕ} (hn : Odd n) (hm163 : 163 ≤ n.primeFactors.card) :
  ...
  have hnpowfact : 2 ^ m * Nat.factorial m ≤ n :=
    pow_mul_factorial_le_of_card_primeFactors hn
  have hlogn_ge : elementaryU (m : ℝ) ≤ Real.log n :=
    log_ge_elementaryU_of_pow_mul_factorial_le hm1
      hnpowfact
  ...
  have hlogq_ge :
    elementaryU (m : ℝ) ≤
      Real.log (NumberTheory.characterModulus n) := by
    rw [hcastq]
  ...
  have hFmono :
    elementaryF (m : ℝ) ≤
      elementaryF 163 :=
    elementaryF_antitoneOn
      (Set.mem_Ici.mpr (by norm_num only : (11 : ℝ) ≤ 163)) (Set.mem_Ici.mpr hm11R) hm163R
  have hF163 := elementaryF_163_le
  have hFeq :
    elementaryF (m : ℝ) =
      ((m : ℝ) + 1) *
        (Real.log (elementaryU (m : ℝ)) /
          elementaryU (m : ℝ)) := by
  ...
  have hloglogq_ge1 :
    (1 : ℝ) ≤ Real.log (Real.log (NumberTheory.characterModulus n)) := by
    have hge : Real.exp 1 ≤ Real.log (NumberTheory.characterModulus n) :=
      hU_ge_e.trans hlogq_ge
    have hh := Real.log_le_log (Real.exp_pos 1) hge
    rwa [Real.log_exp] at hh
  have hloglogq_pos :
    (0 : ℝ) < Real.log (Real.log (NumberTheory.characterModulus n)) := by linarith
  ...
  have homega_eq : (NumberTheory.characterModulus n).primeFactors.card = m + 1 := by
    have h1 :=
      distinctPrimeFactorCount_characterModulus hn
    rwa [distinctPrimeFactorCount_eq_primeFactors_card,
      distinctPrimeFactorCount_eq_primeFactors_card, ←
      hm_def] at h1
  rw [homega_eq]
...
unconditional. -/
theorem elementaryOmegaStatement_of_finite
    (hfin : ElementaryOmegaFiniteStatement) :
