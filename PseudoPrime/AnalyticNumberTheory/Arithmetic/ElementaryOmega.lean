...
`≥ 2k+5 = 2(k+1)+3`. -/
theorem oddPrime_ge (k : ℕ) : 2 * k + 3 ≤ oddPrime k := by
  induction k with
...
theorem oddPrimorial_ge_pow_mul_factorial (m : ℕ) : 2 ^ m * Nat.factorial m ≤ oddPrimorial m := by
  induction m with
  | zero => norm_num only [oddPrimorial_zero, pow_zero, Nat.factorial_zero, one_mul]
  | succ m ih =>
    ...
    calc
      ...
      _ ≤ oddPrimorial m * oddPrime m := h3
...
theorem pow_mul_factorial_le_of_card_primeFactors {n : ℕ} (hn : Odd n) :
  ...
  (oddPrimorial_ge_pow_mul_factorial n.primeFactors.card).trans
