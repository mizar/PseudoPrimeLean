...
theorem norm_riemannZetaLogContourKernel_good_height_le {x : ℝ} (hx : 0 < x) {H : ℝ} (hH : 8 ≤ H)
    ...
    ‖riemannZetaLogContourKernel x (σ + T * Complex.I)‖ ≤
      qMinusOneZetaLogDerivConst * Real.log (T + 2) ^ 2 * max (x ^ (-(1 : ℝ) / 2)) (x ^ (2 : ℝ)) /
        T ^ 2 := by
  ...
  have h1 :
    ...
    mul_le_mul hderiv_bound hxσ_le hxσpos.le
      (mul_nonneg qMinusOneZetaLogDerivConst_nonneg (by positivity))
  ...
  calc
    ...
    _ ≤
        (qMinusOneZetaLogDerivConst * Real.log (T + 2) ^ 2 *
            max (x ^ (-(1 : ℝ) / 2)) (x ^ (2 : ℝ))) /
          T ^ 2 :=
    ...
    (hgood :
      ∀ ρ : ℂ,
        riemannZeta ρ = 0 →
          |ρ.im - H| ≤ 2 → 1 / (4 * jensenLogConst * Real.log (H + 2)) ≤ |T - ρ.im|)
    {σ : ℝ} (hσ1 : -(1 : ℝ) / 2 ≤ σ) (hσ2 : σ ≤ 2) :
    ‖riemannZetaReciprocalContourKernel x (σ + T * Complex.I)‖ ≤
      qMinusOneZetaLogDerivConst * Real.log (T + 2) ^ 2 * max (x ^ (-(3 : ℝ) / 2)) (x ^ (1 : ℝ)) /
        T ^ 2 := by
  set z : ℂ := σ + T * Complex.I with hz_def
  have hzim : z.im = T := by
    simp only [hz_def, Complex.add_im, Complex.ofReal_im, Complex.mul_im, Complex.ofReal_re,
...
theorem norm_intervalIntegral_riemannZetaLogContourKernel_good_height_le {x : ℝ} (hx : 0 < x)
    ...
    ‖∫ σ in lam..tau, riemannZetaLogContourKernel x (σ + T * Complex.I)‖ ≤
      qMinusOneZetaLogDerivConst * Real.log (T + 2) ^ 2 * max (x ^ (-(1 : ℝ) / 2)) (x ^ (2 : ℝ)) /
          T ^ 2 *
        (tau - lam) := by
  have hbound :=
    intervalIntegral.norm_integral_le_of_norm_le_const (a := lam) (b := tau) (f := fun σ : ℝ =>
      riemannZetaLogContourKernel x (σ + T * Complex.I)) (C :=
      qMinusOneZetaLogDerivConst * Real.log (T + 2) ^ 2 * max (x ^ (-(1 : ℝ) / 2)) (x ^ (2 : ℝ)) /
        T ^ 2)
      (by
        ...
        rintro σ ⟨hσ1, hσ2⟩
        exact
          norm_riemannZetaLogContourKernel_good_height_le hx hH hT hgood (le_trans hlam hσ1.le)
            (hσ2.trans htau))
    ...
    (hgood :
      ∀ ρ : ℂ,
        riemannZeta ρ = 0 →
          |ρ.im - H| ≤ 2 → 1 / (4 * jensenLogConst * Real.log (H + 2)) ≤ |T - ρ.im|)
    {lam tau : ℝ} (hlam : -(1 : ℝ) / 2 ≤ lam) (hlamtau : lam ≤ tau) (htau : tau ≤ 2) :
    ‖∫ σ in lam..tau, riemannZetaReciprocalContourKernel x (σ + T * Complex.I)‖ ≤
      qMinusOneZetaLogDerivConst * Real.log (T + 2) ^ 2 * max (x ^ (-(3 : ℝ) / 2)) (x ^ (1 : ℝ)) /
          T ^ 2 *
        (tau - lam) := by
  have hbound :=
    intervalIntegral.norm_integral_le_of_norm_le_const (a := lam) (b := tau) (f := fun σ : ℝ =>
      riemannZetaReciprocalContourKernel x (σ + T * Complex.I)) (C :=
      qMinusOneZetaLogDerivConst * Real.log (T + 2) ^ 2 * max (x ^ (-(3 : ℝ) / 2)) (x ^ (1 : ℝ)) /
        T ^ 2)
      (by
        rw [Set.uIoc_of_le hlamtau]
        rintro σ ⟨hσ1, hσ2⟩
        exact
          norm_riemannZetaReciprocalContourKernel_good_height_le hx hH hT hgood
            (le_trans hlam hσ1.le) (hσ2.trans htau))
  rwa [abs_of_nonneg (by linarith only [hlamtau] : (0 : ℝ) ≤ tau - lam)] at hbound
...
theorem tendsto_intervalIntegral_riemannZetaLogContourKernel_goodHeightSeq {x : ℝ} (hx : 0 < x)
    ...
    Filter.Tendsto
      (fun j : ℕ =>
        ∫ σ in lam..tau, riemannZetaLogContourKernel x (σ + goodHeightSeq j * Complex.I))
      Filter.atTop (nhds 0) := by
  set M : ℝ := max (x ^ (-(1 : ℝ) / 2)) (x ^ (2 : ℝ)) with hM_def
  have hbtend :
    Filter.Tendsto (fun j : ℕ => Real.log (goodHeightSeq j + 2) ^ 2 / (goodHeightSeq j) ^ 2)
      Filter.atTop (nhds 0) :=
    tendsto_log_add_two_sq_div_sq_atTop.comp tendsto_goodHeightSeq_atTop
  have hg :
    ∀ T : ℝ,
  ...
  have hatend :
    Filter.Tendsto
      (fun j : ℕ =>
        qMinusOneZetaLogDerivConst * Real.log (goodHeightSeq j + 2) ^ 2 * M /
            (goodHeightSeq j) ^ 2 *
          (tau - lam))
      Filter.atTop (nhds 0) := by
    simp_rw [hg]
    simpa only [mul_zero] using hbtend.const_mul (qMinusOneZetaLogDerivConst * M * (tau - lam))
  exact
    squeeze_zero_norm
      (fun j =>
        norm_intervalIntegral_riemannZetaLogContourKernel_good_height_le hx
          (le_add_of_nonneg_right (Nat.cast_nonneg j)) (goodHeightSeq_mem j) (goodHeightSeq_good j)
          hlam hlamtau htau)
      hatend
...
theorem tendsto_intervalIntegral_riemannZetaReciprocalContourKernel_goodHeightSeq {x : ℝ}
    ...
    Filter.Tendsto
      (fun j : ℕ =>
        ∫ σ in lam..tau, riemannZetaReciprocalContourKernel x (σ + goodHeightSeq j * Complex.I))
      Filter.atTop (nhds 0) := by
  ...
  exact
    squeeze_zero_norm
      (fun j =>
        norm_intervalIntegral_riemannZetaReciprocalContourKernel_good_height_le hx
          (le_add_of_nonneg_right (Nat.cast_nonneg j)) (goodHeightSeq_mem j) (goodHeightSeq_good j)
          hlam hlamtau htau)
      hatend
