...
noncomputable def goodHeightSeq (j : ℕ) : ℝ :=
  (exists_good_height (H := 8 + (j : ℝ))
      (le_add_of_nonneg_right (Nat.cast_nonneg j))).choose

theorem goodHeightSeq_mem (j : ℕ) : goodHeightSeq j ∈ Set.Icc (8 + (j : ℝ)) (8 + (j : ℝ) + 1) :=
  (exists_good_height (H := 8 + (j : ℝ))
      (le_add_of_nonneg_right (Nat.cast_nonneg j))).choose_spec.1
    ...
    ∀ ρ : ℂ,
      riemannZeta ρ = 0 →
        ...
        1 /
            (4 * jensenLogConst *
              Real.log (8 + (j : ℝ) + 2)) ≤
          |goodHeightSeq j - ρ.im| :=
  (exists_good_height (H := 8 + (j : ℝ))
      (le_add_of_nonneg_right (Nat.cast_nonneg j))).choose_spec.2
