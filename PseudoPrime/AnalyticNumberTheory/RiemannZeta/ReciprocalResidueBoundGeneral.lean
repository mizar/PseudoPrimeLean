...
theorem re_sum_riemannZetaReciprocalZeroContribution_ge_of_riemannHypothesis
  ...
  have hStriv : ∀ ρ ∈ St, ∃ n : ℕ, ρ = -2 * ((n : ℂ) + 1) := fun ρ hρ =>
    exists_nat_eq_neg_two_mul_add_one_of_riemannZeta_zero_re_neg
      (hStz ρ hρ).2 (hStz ρ hρ).1
  have htriv :
    -riemannReciprocalTrivialZeroSeries x ≤
      (∑ ρ ∈ St,
          riemannZetaReciprocalZeroContribution x
      ...
      ∀ ρ ∈ St,
        (riemannZetaReciprocalZeroContribution x
          ...
          -(x⁻¹ ^ (2 * (trivialZeroIndex ρ + 1) + 1) /
              ((2 * (trivialZeroIndex ρ + 1) : ℝ) *
                (2 * (trivialZeroIndex ρ + 1) +
                  1))) := by
      intro ρ hρ
      have hspec := trivialZeroIndex_spec (hStriv ρ hρ)
      conv_lhs => rw [hspec]
      rw [riemannZetaReciprocalZeroContribution_neg_two_mul_nat_add_one
          hxpos,
        Complex.neg_re, Complex.ofReal_re]
    rw [Finset.sum_congr rfl heq, Finset.sum_neg_distrib]
    linarith [sum_reciprocalTrivialZeroTerm_le hx St
        hStriv]
  have hnontriv :
    -(2 * riemannZeroMass / Real.sqrt x) ≤
      (∑ ρ ∈ Sn,
    ...
    have hbound :
      ...
      rw [norm_riemannZetaReciprocalZeroContribution_of_rh
          hRH hxpos (hSnz ρ hρ).1 (hSnz ρ hρ).2] at h2
      linarith [h2.1]
    have hsum_bound :
      -(∑ ρ ∈ Sn,
            (riemannZetaZeroMultiplicity ρ : ℝ) /
    ...
    have hzm :=
      tsum_riemannXiZeroMultiplicity_invNormSq_eq_two_mul_riemannZeroMass_of_riemannHypothesis
        hRH
    have hxieq :
      ∀ ρ ∈ Sn,
        (riemannZetaZeroMultiplicity ρ : ℝ) /
      ...
      intro ρ hρ
      have hxi0 :=
        riemannXi_eq_zero_of_riemannZeta_zero_re_nonneg
          (hSnz ρ hρ).1 (hSnz ρ hρ).2
      rw [ite_eq_left hxi0,
        riemannXiZeroMultiplicity_eq_riemannZetaZeroMultiplicity_of_zero
          hxi0]
    have hsum_eq :
      (∑ ρ ∈ Sn,
          (riemannZetaZeroMultiplicity ρ : ℝ) /
    ...
    have heq2 :
      (2 : ℝ) * riemannZeroMass / Real.sqrt x =
        (Real.sqrt x)⁻¹ * (2 * riemannZeroMass) := by
      ring
    linarith [hmul, heq2]
...
theorem re_riemannZetaReciprocalContourResidueLedger_unifiedTau_ge_of_riemannHypothesis
  ...
  have hxpos : 0 < x := by linarith
  rw [riemannZetaReciprocalContourResidueLedger_unifiedTau_eq
      hτ,
    Complex.add_re, Complex.add_re,
    riemannZetaReciprocalResidueAtZero_eq,
    riemannZetaReciprocalResidueAtOne_eq hxpos]
  ...
  have hzero_ledger :=
    re_sum_riemannZetaReciprocalZeroContribution_ge_of_riemannHypothesis
      hRH hx
...
theorem re_integral_riemannZetaReciprocalContourKernel_two_ge_of_riemannHypothesis
  ...
  · simpa only [Complex.ofReal_mul, Complex.ofReal_ofNat, smul_eq_mul] using
      tendsto_riemannZetaReciprocalContourResidueLedger_atTop
