...
theorem re_sum_riemannZetaLogZeroContribution_nontrivial_ge_of_riemannHypothesis
  ...
  have hbound :
    ...
    rw [norm_riemannZetaLogZeroContribution_of_rh hRH
        hx (hSzero ρ hρ) (hSre ρ hρ)] at h2
    linarith [h2.1]
  have hsum_bound :
    -(∑ ρ ∈ S,
          Real.sqrt x *
      ...
      ∀ ρ ∈ St,
        (riemannZetaLogZeroContribution x ρ).re =
          -(x⁻¹ ^ (2 * (trivialZeroIndex ρ + 1)) /
              (4 *
                ((trivialZeroIndex ρ : ℝ) + 1) ^
                  2)) := by
      intro ρ hρ
      have hspec := trivialZeroIndex_spec (hStriv ρ hρ)
      conv_lhs => rw [hspec]
      rw [riemannZetaLogZeroContribution_neg_two_mul_nat_add_one
          hxpos,
