...
theorem dirichletLogFiniteContourIdentity {N : ℕ} [NeZero N] {x : ℝ} (hx : 0 < x)
    ...
    RectangleGeometry.rectangleBoundaryIntegral
        (dirichletLogContourKernel x χ) z w =
      ∑
        s ∈
          dirichletLFunctionSingularitiesInRectangle
            χ hne z w,
  ...
  have hcoordint :=
    dirichletLogKernelCoordinateIntegrable hx
      hne grid havoid
  have hgridint :
  ...
  have hsum :
    RectangleGeometry.rectangleBoundaryIntegral
      ...
      ∑ cell ∈ cells,
        RectangleGeometry.rectangleBoundaryIntegral
          (dirichletLogContourKernel x χ) cell.1
          cell.2 := by
  ...
  have hsingular_res :
    ...
    have hpointHyp :=
      (mem_dirichletLFunctionSingularitiesInRectangle_iff.mp
          hpointS).2
    obtain ⟨Rc, hRc, hcert⟩ :=
      exists_radius_forall_dirichletRectangleBoundaryIntegral_log_residueAt
    ...
    have hdiffCell :
      ∀ y ∈ Rectangle.rectangleClosedBox cell.1 cell.2,
        y ∉ S →
          DifferentiableAt ℂ
            (dirichletLogContourKernel x χ) y :=
    ...
    have hcoordint :=
      assignment.kernelCoordinateIntegrable
        (dirichletLogContourKernel x χ) hcell
        hdiffCell xs ys hxmem hymem havoidSq.1 havoidSq.2
    have hgrid3 :
      RectangleGeometry.RectangleGridSubdivisionIntegrable
        (dirichletLogContourKernel x χ) cell.1
...
theorem dirichletLogFiniteContourIdentity_heightSeq {N : ℕ} [NeZero N] (hN2 : 2 ≤ N)
  ...
  exact
    dirichletLogFiniteContourIdentity hx
      ...
      ∑
        s ∈
          dirichletLFunctionSingularitiesInRectangle
            χ hne z w,
        dirichletLogResidueAt hne x s := by
  rw [dirichletLogFiniteContourIdentity hx
        ...
        s ∈
          dirichletLFunctionSingularitiesInRectangle
            ...
            (primitiveReciprocalUpperCorner hN2
              hGRH hprimitive hne hinv hquad k),
        dirichletLogResidueAt hne x s := by
  rw [dirichletLogFiniteContourIdentity_heightSeq
...
theorem dirichletLogFiniteContourIdentity_heightSeq_normalized_of_grh {N : ℕ} [NeZero N]
  ...
  exact
    dirichletLogFiniteContourIdentity_normalized
      hx hprimitive hne hre him
      (primitiveHeightSeq_singularities_mem_open_of_grh
