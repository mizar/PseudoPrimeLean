...
/--
For a character of nonzero level, `x > 0`, and `τ > 1`, the twisted logarithmic
Mellin-weighted von Mangoldt series equals `(2π)⁻¹` times the vertical kernel integral.
Termwise Mellin inversion and the summable integral-norm majorant justify interchange.
The twisted von Mangoldt Dirichlet series identifies the logarithmic derivative.
This supplies the integral identity before the finite-sum collapse.
-/
theorem characterLogWeightedTerm_tsum_eq_integral {N : ℕ} [NeZero N] (χ : DirichletCharacter ℂ N)
  ...
  have hnormH :
    ...
    calc
      ...
      _ ≤ ArithmeticFunction.vonMangoldt n * 1 * (n : ℝ) ^ (-τ) * x ^ τ := by
        rw [hΛ, hn_cpow, hx_cpow]
        gcongr
      _ = ArithmeticFunction.vonMangoldt n * (n : ℝ) ^ (-τ) * x ^ τ := by ring
  ...
  have hGnorm_le :
    ...
    intro n hn
    have hcongr :
      ∀ y : ℝ, ‖G n y‖ ≤ (ArithmeticFunction.vonMangoldt n * (n : ℝ) ^ (-τ) * x ^ τ) * ‖K y‖ := by
      intro y
      rw [hG_def, norm_mul, mul_comm]
      exact mul_le_mul_of_nonneg_right (hnormH n hn y) (norm_nonneg _)
    calc
      ∫ y : ℝ, ‖G n y‖ ≤
          ∫ y : ℝ, (ArithmeticFunction.vonMangoldt n * (n : ℝ) ^ (-τ) * x ^ τ) * ‖K y‖ :=
      ...
      _ = (ArithmeticFunction.vonMangoldt n * (n : ℝ) ^ (-τ) * x ^ τ) * ∫ y : ℝ, ‖K y‖ := by
        rw [MeasureTheory.integral_const_mul]
  have hSummableL : Summable (fun n : ℕ ↦ ∫ y : ℝ, ‖G n y‖) := by
    ...
    · calc
        ∫ y : ℝ, ‖G n y‖ ≤
            (ArithmeticFunction.vonMangoldt n * (n : ℝ) ^ (-τ) * x ^ τ) * ∫ y : ℝ, ‖K y‖ :=
          hGnorm_le n hn
        _ = ArithmeticFunction.vonMangoldt n * (n : ℝ) ^ (-τ) * (x ^ τ * ∫ y : ℝ, ‖K y‖) := by ring
  ...
  have hLHS :
    ...
    rcases eq_or_ne n 0 with rfl | hn
    · simp only [ArithmeticFunction.map_zero, Complex.ofReal_zero, Nat.cast_zero, zero_mul,
        CharP.cast_eq_zero, zero_div, mul_inv_rev, hG_def, hH_def, neg_add_rev, mul_zero,
...
/--
For any character of nonzero level, `x > 0`, and `τ > 1`, the finite logarithmic
weighted character sum equals `(2π)⁻¹` times the right-vertical kernel integral.
Combine the finite-sum collapse with `characterLogWeightedTerm_tsum_eq_integral`.
This identifies the right edge in the logarithmic contour limit.
-/
