...
theorem exists_dirichletRectangleBoundaryIntegral_reciprocal_eq_two_pi_I_mul_zeroContribution
  ...
  have hh :
    AnalyticAt ℂ
      (dirichletReciprocalZeroRegularization x ρ (dirichletLFunctionZeroMultiplicity χ ρ) g) ρ :=
    analyticAt_dirichletReciprocalZeroRegularization hx hρ0 hρ1 _ hganalytic hgzero
...
theorem dirichletReciprocalFiniteContourIdentity_heightSeq_of_grh {N : ℕ} [NeZero N] (hN2 : 2 ≤ N)
  ...
  exact dirichletReciprocalFiniteContourIdentity hx hprimitive hne hre him hopen
...
theorem dirichletReciprocalFiniteContourIdentity_heightSeq {N : ℕ} [NeZero N] (hN2 : 2 ≤ N)
  ...
  exact
    dirichletReciprocalFiniteContourIdentity hx hprimitive hne hre him
...
theorem dirichletReciprocalFiniteContourIdentity_heightSeq_normalized {N : ℕ} [NeZero N]
      ...
      ∑
        s ∈
          dirichletLFunctionSingularitiesInRectangle χ hne
            ...
            (primitiveReciprocalUpperCorner hN2 hGRH hprimitive hne hinv hquad k),
        dirichletReciprocalResidueAt hne x s := by
  rw [dirichletReciprocalFiniteContourIdentity_heightSeq hN2 hGRH hprimitive hne hinv hquad hx A k
      hA,
    Finset.mul_sum]
  ...
  calc
    (-Complex.I / (2 * (Real.pi : ℂ))) *
      ...
      by ring
    _ = dirichletReciprocalResidueAt hne x s := by rw [hcoeff, one_mul]
