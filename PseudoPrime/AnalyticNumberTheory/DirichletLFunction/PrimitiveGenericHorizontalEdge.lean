...
theorem intervalIntegrable_dirichletReciprocalContourKernel_horizontalHeightSeq_of_grh {N : ℕ}
  ...
  have h :=
    continuous_dirichletReciprocalContourKernel_horizontalHeightSeq_of_grh hN2 hGRH hprimitive hne
      hinv hx k
...
theorem intervalIntegrable_dirichletReciprocalContourKernel_central_of_grh {N : ℕ} [NeZero N]
  ...
  exact
    intervalIntegrable_dirichletReciprocalContourKernel_horizontalHeightSeq_of_grh hN2 hGRH
      hprimitive hne hinv hx k (-2) 2
...
theorem intervalIntegrable_dirichletReciprocalContourKernel_farLeft_of_grh (A : ℕ) {N : ℕ}
  ...
  exact
    intervalIntegrable_dirichletReciprocalContourKernel_horizontalHeightSeq_of_grh hN2 hGRH
      hprimitive hne hinv hx k (-(A : ℝ) - 1 / 2) (-2)
...
theorem intervalIntegral_add_adjacent_dirichletReciprocalKernel_of_grh (A : ℕ) {N : ℕ} [NeZero N]
  ...
  · exact
      (intervalIntegrable_dirichletReciprocalContourKernel_farLeft_of_grh A hN2 hGRH hprimitive hne
          hinv hx k).1
  · exact
      (intervalIntegrable_dirichletReciprocalContourKernel_central_of_grh hN2 hGRH hprimitive hne
          hinv hx k).1
...
theorem intervalIntegrable_dirichletReciprocalKernel_full_of_grh (A : ℕ) {N : ℕ} [NeZero N]
  ...
  exact
    (intervalIntegrable_dirichletReciprocalContourKernel_horizontalHeightSeq_of_grh hN2 hGRH
        hprimitive hne hinv hx k (-(A : ℝ) - 1 / 2) 2).1
...
theorem intervalIntegrable_dirichletReciprocalKernel_full_lower_of_grh (A : ℕ) {N : ℕ} [NeZero N]
  ...
  exact
    (intervalIntegrable_dirichletReciprocalContourKernel_horizontalHeightSeq_of_grh hN2 hGRH
        hprimitive hne hinv hx k (-(A : ℝ) - 1 / 2) 2).2
...
theorem intervalIntegrable_dirichletReciprocalKernel_full_pair_of_grh (A : ℕ) {N : ℕ} [NeZero N]
    ...
    IntervalIntegrable
      ...
      IntervalIntegrable
        ...
        MeasureTheory.volume (-(A : ℝ) - 1 / 2) 2 := by
  exact
    ⟨intervalIntegrable_dirichletReciprocalKernel_full_of_grh A hN2 hGRH hprimitive hne hinv hx k,
      intervalIntegrable_dirichletReciprocalKernel_full_lower_of_grh A hN2 hGRH hprimitive hne hinv
        hx k⟩
        ...
        ∫ σ in (-(A : ℝ) - 1 / 2)..2,
          dirichletReciprocalContourKernel x χ
            ((σ : ℂ) -
              primitiveHorizontalHeightSeq_of_grh hN2 hGRH hprimitive hne hinv k * Complex.I) := by
  exact
    ⟨intervalIntegral_add_adjacent_dirichletReciprocalKernel_of_grh A hN2 hGRH hprimitive hne hinv
        hx k,
      intervalIntegral_add_adjacent_dirichletReciprocalKernel_lower_of_grh A hN2 hGRH hprimitive hne
        hinv hx k⟩
...
theorem tendsto_primitiveHorizontalHeightSeq_reciprocalKernel_horizontal_integral_of_grh (A : ℕ)
  ...
  have hfar :=
    tendsto_primitiveHorizontalHeightSeq_reciprocalKernel_farLeft_integral_of_grh A hA hN2 hGRH
      hprimitive hne hinv hx
  have hcentral :=
    tendsto_primitiveHorizontalHeightSeq_reciprocalKernel_central_integral_of_grh hN2 hGRH
      hprimitive hne hinv hx
  have hsplit := fun k : ℕ =>
...
theorem tendsto_normalized_primitiveHorizontalReciprocalEdge_of_grh (A : ℕ) (hA : 2 ≤ A) {N : ℕ}
    ...
    Filter.Tendsto
      ...
      Filter.atTop (nhds 0) := by
  have hhoriz :=
    tendsto_primitiveHorizontalHeightSeq_reciprocalKernel_horizontal_integral_of_grh A hA hN2 hGRH
      hprimitive hne hinv hx
...
theorem tendsto_normalized_dirichletReciprocalBoundary_heightSeq_of_grh {N : ℕ} [NeZero N]
  ...
  have heq :
    ...
    have hzim :
      (primitiveHeightSeqLowerCorner_of_grh hN2 hGRH hprimitive hne hinv A k).im =
        -(primitiveHorizontalHeightSeq_of_grh hN2 hGRH hprimitive hne hinv k) := by
      rw [primitiveHeightSeqLowerCorner_of_grh]
    ...
    have hwre : (primitiveHeightSeqUpperCorner_of_grh hN2 hGRH hprimitive hne hinv k).re = 2 := by
  ...
  have hhoriz :=
    tendsto_normalized_primitiveHorizontalReciprocalEdge_of_grh A hA hN2 hGRH hprimitive hne hinv hx
  have hright :
    Filter.Tendsto
      (fun k : ℕ =>
        ∫ t in
