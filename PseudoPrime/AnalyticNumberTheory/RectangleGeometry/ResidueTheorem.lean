...
noncomputable def finiteRegularCells (S : Finset ℂ) (cells : Finset (ℂ × ℂ)) : Finset (ℂ × ℂ) := by
  classical
    ...
    cells.filter fun cell ↦
      ¬CellContainsSingularity S cell
...
theorem mem_regularCells_iff {S : Finset ℂ} {cells : Finset (ℂ × ℂ)} {cell : ℂ × ℂ} :
    cell ∈ finiteRegularCells S cells ↔
      cell ∈ cells ∧
        ¬CellContainsSingularity S cell := by
  classical
    simp only [finiteRegularCells,
    Finset.mem_filter]
...
structure SingularCellAssignment (S : Finset ℂ) (cells : Finset (ℂ × ℂ)) where
  ...
  distinct_points :
    ∀ cell ∈ finiteSingularCells S cells,
      ∀ other ∈ finiteSingularCells S cells,
...
theorem GridInteriorSeparation.toSeparation {S : Finset ℂ} {cells : Finset (ℂ × ℂ)}
    ...
    GridSingularitySeparation S cells := by
...
noncomputable def GridSingularitySeparation.toAssignment {S : Finset ℂ} {cells : Finset (ℂ × ℂ)}
    (separation :
      GridSingularitySeparation S cells) :
    SingularCellAssignment S cells := by
  classical
  let pointOfCell : ℂ × ℂ → ℂ := fun cell ↦
    Classical.epsilon fun s ↦
      s ∈ S ∧ s ∈ Rectangle.rectangleClosedBox cell.1 cell.2
  have point_spec (cell : ℂ × ℂ)
    (hcell : CellContainsSingularity S cell) :
    pointOfCell cell ∈ S ∧
      pointOfCell cell ∈
  ...
  · intro cell hcell
    exact
      (point_spec cell
          (mem_singularCells_iff.mp hcell).2).1
  · intro cell hcell
    exact
      (point_spec cell
          (mem_singularCells_iff.mp hcell).2).2
  ...
  · intro cell hcell other hother heq
    have hcell' := mem_singularCells_iff.mp hcell
    have hother' :=
      mem_singularCells_iff.mp hother
    exact
...
noncomputable def GridInteriorSeparation.toAssignment {S : Finset ℂ} {cells : Finset (ℂ × ℂ)}
    (interior : GridInteriorSeparation S cells) :
    SingularCellAssignment S cells :=
...
theorem SingularCellAssignment.image_pointOfCell_eq {S : Finset ℂ} {cells : Finset (ℂ × ℂ)}
    (assignment :
      SingularCellAssignment S cells) :
    (finiteSingularCells S cells).image
        assignment.pointOfCell =
      S := by
...
theorem rectangleBoundaryIntegral_eq_sum_res (f res : ℂ → ℂ) (S : Finset ℂ) (cells : Finset (ℂ × ℂ))
    ...
    (hdiff :
      ∀ cell ∈ cells,
        ∀ x ∈ Rectangle.rectangleClosedBox cell.1 cell.2,
          x ∉ S → DifferentiableAt ℂ f x)
    (hsingular_res :
      ∀ cell ∈ finiteSingularCells S cells,
        rectangleBoundaryIntegral f cell.1
            cell.2 =
          res (assignment.pointOfCell cell)) :
    rectangleBoundaryIntegral f z w =
      ∑ s ∈ S, res s := by
  have hregular_zero :
    ∀ cell ∈ finiteRegularCells S cells,
    ...
    have hdisjoint :
      ¬CellContainsSingularity S cell := hcell'.2
    apply
  ...
  have hcombine :=
    sum_finiteRegularCells_add_sum_finiteSingularCells
      S cells
      (fun cell ↦
        rectangleBoundaryIntegral f cell.1
          cell.2)
