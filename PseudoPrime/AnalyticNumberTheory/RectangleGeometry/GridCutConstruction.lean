...
theorem StrictGridCuts.xcoordinates_pairwise {z w : ℂ} (grid : StrictGridCuts z w) :
    (z.re :: grid.xcuts ++ [w.re]).Pairwise (· < ·) :=
  endpointAugmentedCoordinates_pairwise grid.re_lt grid.xcuts_pairwise grid.xcuts_inside

/-- The endpoint-augmented imaginary coordinates of a strict grid remain strictly increasing. -/
theorem StrictGridCuts.ycoordinates_pairwise {z w : ℂ} (grid : StrictGridCuts z w) :
    (z.im :: grid.ycuts ++ [w.im]).Pairwise (· < ·) :=
  endpointAugmentedCoordinates_pairwise grid.im_lt grid.ycuts_pairwise grid.ycuts_inside

/-- The endpoint-augmented real coordinates of a strict grid are duplicate-free. -/
theorem StrictGridCuts.xcoordinates_nodup {z w : ℂ} (grid : StrictGridCuts z w) :
    (z.re :: grid.xcuts ++ [w.re]).Nodup :=
  endpointAugmentedCoordinates_nodup grid.re_lt grid.xcuts_pairwise grid.xcuts_inside

/-- The endpoint-augmented imaginary coordinates of a strict grid are duplicate-free. -/
theorem StrictGridCuts.ycoordinates_nodup {z w : ℂ} (grid : StrictGridCuts z w) :
    (z.im :: grid.ycuts ++ [w.im]).Nodup :=
  endpointAugmentedCoordinates_nodup grid.im_lt grid.ycuts_pairwise grid.ycuts_inside

/-- Every endpoint-augmented real grid coordinate lies in the outer unordered interval. -/
theorem StrictGridCuts.xcoordinate_mem_uIcc {z w : ℂ} (grid : StrictGridCuts z w) {c : ℝ}
...
theorem StrictGridCuts.ycoordinate_mem_uIcc {z w : ℂ} (grid : StrictGridCuts z w) {c : ℝ}
...
theorem StrictGridCuts.cells_nodup {z w : ℂ} (grid : StrictGridCuts z w) :
    (rectangleGridCells z w grid.xcuts grid.ycuts).Nodup :=
  rectangleGridCells_nodup grid.xcoordinates_nodup grid.ycoordinates_nodup

/-- Distinct cells of a strict grid have disjoint open rectangles. -/
theorem StrictGridCuts.open_disjoint {z w : ℂ} (grid : StrictGridCuts z w) {cell other : ℂ × ℂ}
    (hcell : cell ∈ (rectangleGridCells z w grid.xcuts grid.ycuts).toFinset)
    (hother : other ∈ (rectangleGridCells z w grid.xcuts grid.ycuts).toFinset)
    (hne : cell ≠ other) :
    Disjoint (rectangleOpenBox cell.1 cell.2) (rectangleOpenBox other.1 other.2) :=
  rectangleGridCells_openBox_disjoint grid.xcoordinates_pairwise grid.ycoordinates_pairwise
...
def StrictGridCuts.LedgerAvoidsCoordinates {z w : ℂ} (grid : StrictGridCuts z w) (S : Finset ℂ) :
    Prop :=
...
def StrictGridCuts.LedgerAvoidsCuts {z w : ℂ} (grid : StrictGridCuts z w) (S : Finset ℂ) : Prop :=
...
-/
def StrictGridCuts.LedgerPairsSeparatedByCuts {z w : ℂ} (grid : StrictGridCuts z w) (S : Finset ℂ) :
    Prop :=
  ∀ s ∈ S,
    ∀ t ∈ S,
      s ≠ t →
...
theorem StrictGridCuts.ledgerAvoidsCoordinates_of_open {z w : ℂ} (grid : StrictGridCuts z w)
    {S : Finset ℂ} (hopen : ∀ s ∈ S, s ∈ rectangleOpenBox z w) (hcuts : grid.LedgerAvoidsCuts S) :
    grid.LedgerAvoidsCoordinates S := by
  intro s hs
...
theorem StrictGridCuts.point_mem_open_of_avoidsCoordinates {z w : ℂ} (grid : StrictGridCuts z w)
    {S : Finset ℂ} (havoid : grid.LedgerAvoidsCoordinates S) {s : ℂ} (hs : s ∈ S) {cell : ℂ × ℂ}
    (hcell : cell ∈ (rectangleGridCells z w grid.xcuts grid.ycuts).toFinset)
    (hsclosed : s ∈ Rectangle.rectangleClosedBox cell.1 cell.2) :
    s ∈ rectangleOpenBox cell.1 cell.2 :=
  mem_rectangleGridCell_openBox_of_avoids_coordinates grid.xcoordinates_pairwise
    grid.ycoordinates_pairwise (by simpa only [List.mem_toFinset] using hcell) hsclosed
    (havoid s hs).1 (havoid s hs).2

/-- Pair-separating cuts make each strict grid cell contain at most one point of `S`. -/
theorem StrictGridCuts.cell_point_unique_of_pairSeparated {z w : ℂ} (grid : StrictGridCuts z w)
    {S : Finset ℂ} (havoid : grid.LedgerAvoidsCoordinates S)
    (hseparated : grid.LedgerPairsSeparatedByCuts S) (cell : ℂ × ℂ)
    (hcell : cell ∈ (rectangleGridCells z w grid.xcuts grid.ycuts).toFinset) {s : ℂ} (hs : s ∈ S)
    (hsclosed : s ∈ Rectangle.rectangleClosedBox cell.1 cell.2) {t : ℂ} (ht : t ∈ S)
    (htclosed : t ∈ Rectangle.rectangleClosedBox cell.1 cell.2) : s = t := by
  by_contra hne
  ...
  apply hne
  apply
    eq_of_mem_same_gridCell_openBox_of_coordinateSeparated grid.xcoordinates_pairwise
      grid.ycoordinates_pairwise (by simpa only [List.mem_toFinset] using hcell) hsopen htopen
...
theorem StrictGridCuts.point_covered {z w : ℂ} (grid : StrictGridCuts z w) {s : ℂ}
    (hs : s ∈ Rectangle.rectangleClosedBox z w) :
    ∃ cell ∈ (rectangleGridCells z w grid.xcuts grid.ycuts).toFinset,
      s ∈ Rectangle.rectangleClosedBox cell.1 cell.2 := by
  obtain ⟨cell, hcell, hscell⟩ :=
    exists_mem_rectangleGridCell_closedBox grid.re_lt grid.im_lt grid.xcuts_pairwise
      grid.ycuts_pairwise grid.xcuts_inside grid.ycuts_inside hs
...
theorem StrictGridCuts.interiorSeparationOfCutCertificates {z w : ℂ} (grid : StrictGridCuts z w)
    {S : Finset ℂ} (hopen : ∀ s ∈ S, s ∈ rectangleOpenBox z w)
    (hclosed : ∀ s ∈ S, s ∈ Rectangle.rectangleClosedBox z w) (hcuts : grid.LedgerAvoidsCuts S)
    (hseparated : grid.LedgerPairsSeparatedByCuts S) :
    GridInteriorSeparation S (rectangleGridCells z w grid.xcuts grid.ycuts).toFinset := by
...
noncomputable def realCutCandidates (S : Finset ℂ) : Finset ℝ :=
  (orderedRealPairs S).image fun p ↦ avoidingCut (singularityRealCoordinates S) p.1.re p.2.re
...
noncomputable def imagCutCandidates (S : Finset ℂ) : Finset ℝ :=
  (orderedImagPairs S).image fun p ↦ avoidingCut (singularityImagCoordinates S) p.1.im p.2.im
...
theorem realCuts_pairwise (S : Finset ℂ) : (realCuts S).Pairwise (· < ·) :=
  (Finset.sortedLT_sort (realCutCandidates S)).pairwise

/-- The sorted imaginary cut list of `S` is strictly increasing. -/
theorem imagCuts_pairwise (S : Finset ℂ) : (imagCuts S).Pairwise (· < ·) :=
  (Finset.sortedLT_sort (imagCutCandidates S)).pairwise

/-- Every generated real cut of `S` avoids every real coordinate of `S`. -/
theorem realCuts_avoid {S : Finset ℂ} {u : ℝ} (hu : u ∈ realCuts S) {s : ℂ} (hs : s ∈ S) :
    u ≠ s.re := by
  classical
  rw [realCuts, Finset.mem_sort] at hu
  obtain ⟨p, hp, rfl⟩ := Finset.mem_image.mp hu
  have hp' := Finset.mem_filter.mp hp
  have havoid := (avoidingCut_spec (singularityRealCoordinates S) hp'.2).2.2
  intro heq
  exact havoid (heq ▸ Finset.mem_image.mpr ⟨s, hs, rfl⟩)

/-- Every generated imaginary cut of `S` avoids every imaginary coordinate of `S`. -/
theorem imagCuts_avoid {S : Finset ℂ} {v : ℝ} (hv : v ∈ imagCuts S) {s : ℂ} (hs : s ∈ S) :
    v ≠ s.im := by
  classical
  rw [imagCuts, Finset.mem_sort] at hv
  obtain ⟨p, hp, rfl⟩ := Finset.mem_image.mp hv
  have hp' := Finset.mem_filter.mp hp
  have havoid := (avoidingCut_spec (singularityImagCoordinates S) hp'.2).2.2
  intro heq
  exact havoid (heq ▸ Finset.mem_image.mpr ⟨s, hs, rfl⟩)
...
theorem generatedCuts_pair_separated {S : Finset ℂ} {s t : ℂ} (hs : s ∈ S) (ht : t ∈ S)
    (hne : s ≠ t) :
    (∃ u ∈ realCuts S, (s.re < u ∧ u < t.re) ∨ (t.re < u ∧ u < s.re)) ∨
      ∃ v ∈ imagCuts S, (s.im < v ∧ v < t.im) ∨ (t.im < v ∧ v < s.im) := by
  classical
  ...
  · have him : s.im ≠ t.im := fun him ↦ hne (Complex.ext hre him)
    ...
    · refine
        Or.inr
          ⟨avoidingCut (singularityImagCoordinates S) s.im t.im, ?_,
            Or.inl ⟨(avoidingCut_spec _ hst).1, (avoidingCut_spec _ hst).2.1⟩⟩
      rw [imagCuts, Finset.mem_sort, imagCutCandidates]
      exact
        Finset.mem_image.mpr
          ⟨(s, t), Finset.mem_filter.mpr ⟨Finset.mem_product.mpr ⟨hs, ht⟩, hst⟩, rfl⟩
    · refine
        Or.inr
          ⟨avoidingCut (singularityImagCoordinates S) t.im s.im, ?_,
            Or.inr ⟨(avoidingCut_spec _ hts).1, (avoidingCut_spec _ hts).2.1⟩⟩
      rw [imagCuts, Finset.mem_sort, imagCutCandidates]
      exact
        Finset.mem_image.mpr
  ...
  · rcases lt_or_gt_of_ne hre with hst | hts
    · refine
        Or.inl
          ⟨avoidingCut (singularityRealCoordinates S) s.re t.re, ?_,
            Or.inl ⟨(avoidingCut_spec _ hst).1, (avoidingCut_spec _ hst).2.1⟩⟩
      rw [realCuts, Finset.mem_sort, realCutCandidates]
      exact
        Finset.mem_image.mpr
          ⟨(s, t), Finset.mem_filter.mpr ⟨Finset.mem_product.mpr ⟨hs, ht⟩, hst⟩, rfl⟩
    · refine
        Or.inl
          ⟨avoidingCut (singularityRealCoordinates S) t.re s.re, ?_,
            Or.inr ⟨(avoidingCut_spec _ hts).1, (avoidingCut_spec _ hts).2.1⟩⟩
      rw [realCuts, Finset.mem_sort, realCutCandidates]
      exact
        Finset.mem_image.mpr
...
theorem realCuts_inside {S : Finset ℂ} {z w : ℂ} (hre : z.re < w.re)
    (hopen : ∀ s ∈ S, s ∈ rectangleOpenBox z w) {u : ℝ} (hu : u ∈ realCuts S) :
    z.re < u ∧ u < w.re := by
  classical
  rw [realCuts, Finset.mem_sort, realCutCandidates] at hu
  obtain ⟨p, hp, rfl⟩ := Finset.mem_image.mp hu
  have hp' := Finset.mem_filter.mp hp
  ...
  have hcut := avoidingCut_spec (singularityRealCoordinates S) hp'.2
...
theorem imagCuts_inside {S : Finset ℂ} {z w : ℂ} (him : z.im < w.im)
    (hopen : ∀ s ∈ S, s ∈ rectangleOpenBox z w) {v : ℝ} (hv : v ∈ imagCuts S) :
    z.im < v ∧ v < w.im := by
  classical
  rw [imagCuts, Finset.mem_sort, imagCutCandidates] at hv
  obtain ⟨p, hp, rfl⟩ := Finset.mem_image.mp hv
  have hp' := Finset.mem_filter.mp hp
  have hpMem := Finset.mem_product.mp hp'.1
  have hopenLeft := hopen p.1 hpMem.1
  ...
  have hcut := avoidingCut_spec (singularityImagCoordinates S) hp'.2
  exact ⟨hleft.trans hcut.1, hcut.2.1.trans hright⟩
...
noncomputable def generatedStrictGridCuts (S : Finset ℂ) (z w : ℂ) (hre : z.re < w.re)
    (him : z.im < w.im) (hopen : ∀ s ∈ S, s ∈ rectangleOpenBox z w) : StrictGridCuts z w
    where
  xcuts := realCuts S
  ycuts := imagCuts S
  re_lt := hre
  ...
  xcuts_inside := fun _ hu ↦ realCuts_inside hre hopen hu
  ycuts_inside := fun _ hv ↦ imagCuts_inside him hopen hv
...
theorem generatedStrictGridCuts_avoidsCuts (S : Finset ℂ) (z w : ℂ) (hre : z.re < w.re)
    (him : z.im < w.im) (hopen : ∀ s ∈ S, s ∈ rectangleOpenBox z w) :
    (generatedStrictGridCuts S z w hre him hopen).LedgerAvoidsCuts S := by
  intro s hs
  exact ⟨fun hu ↦ (realCuts_avoid hu hs) rfl, fun hv ↦ (imagCuts_avoid hv hs) rfl⟩
...
theorem generatedStrictGridCuts_pairsSeparated (S : Finset ℂ) (z w : ℂ) (hre : z.re < w.re)
    (him : z.im < w.im) (hopen : ∀ s ∈ S, s ∈ rectangleOpenBox z w) :
    (generatedStrictGridCuts S z w hre him hopen).LedgerPairsSeparatedByCuts S :=
  fun _ hs _ ht hne ↦ generatedCuts_pair_separated hs ht hne
...
theorem generatedGridInteriorSeparation (S : Finset ℂ) (z w : ℂ) (hre : z.re < w.re)
    ...
    GridInteriorSeparation S
      (rectangleGridCells z w (generatedStrictGridCuts S z w hre him hopen).xcuts
          (generatedStrictGridCuts S z w hre him hopen).ycuts).toFinset :=
  (generatedStrictGridCuts S z w hre him hopen).interiorSeparationOfCutCertificates hopen hclosed
    (generatedStrictGridCuts_avoidsCuts S z w hre him hopen)
    (generatedStrictGridCuts_pairsSeparated S z w hre him hopen)
