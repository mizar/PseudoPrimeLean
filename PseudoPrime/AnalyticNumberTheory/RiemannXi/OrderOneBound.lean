...
theorem xiOrderOneBound_monotoneOn :
    MonotoneOn xiOrderOneBound (Set.Ici (0 : ℝ)) := by
  intro a ha b hb hab
...
theorem one_le_xiOrderOneBound {x : ℝ} (hx : 0 ≤ x) :
    1 ≤ xiOrderOneBound x := by
  have h1 :
    0 ≤
      x + (x + 1) / 2 +
        x * (x + 1) ^ 2 *
    ...
    have :
      0 ≤
        x * (x + 1) ^ 2 *
          RiemannZeta.sawtoothRemainderBound (1 / 2) :=
      mul_nonneg (mul_nonneg hx (by positivity))
        (RiemannZeta.sawtoothRemainderBound_nonneg _)
    linarith
  have h2 :
    0 ≤
...
theorem norm_riemannXi_le_xiOrderOneBound_of_one_half_le_re {s : ℂ} (hs : 1 / 2 ≤ s.re) :
  ...
  · rw [riemannXi_one]
    ...
    rw [hval]
    exact
      le_trans (by norm_num only)
        (one_le_xiOrderOneBound (norm_nonneg _))
  · have hre_half_pos : (0 : ℝ) < s.re / 2 + 1 := by linarith
    ...
    rw [riemannXi_eq_gamma_mul_zetaEntire hs1 hΓne,
      norm_mul, norm_mul]
    ...
    have hfac3 :
      ...
      have hz :=
        RiemannZeta.norm_zetaEntire_le_of_reGt_neg_one (s := s)
          (by linarith)
      have hsaw_le :
        RiemannZeta.sawtoothRemainderBound s.re ≤
          RiemannZeta.sawtoothRemainderBound (1 / 2) :=
        RiemannZeta.sawtoothRemainderBound_antitone
          (by norm_num only) hs
      calc
        ‖RiemannZeta.zetaEntire s‖ ≤
            ‖s‖ + (‖s‖ + 1) / 2 +
...
theorem norm_riemannXi_le_xiOrderOneBound (s : ℂ) :
    ‖riemannXi s‖ ≤
      xiOrderOneBound (‖s‖ + 1) :=
  norm_riemannXi_le_of_forall_one_half_le_re
    xiOrderOneBound_monotoneOn
    (fun _ ht =>
      norm_riemannXi_le_xiOrderOneBound_of_one_half_le_re
        ht)
    s
