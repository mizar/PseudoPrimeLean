...
theorem analyticAt_Gamma_of_re_pos {c : ℂ} (hc : 0 < c.re) : AnalyticAt ℂ Complex.Gamma c := by
  ...
  · intro w hw
    ...
    exact ball_avoids_Gamma_poles_of_re_pos (by linarith only [hc]) (by linarith only [hc]) w hw
...
theorem analyticAt_riemannXiZetaUnit {s : ℂ} (hs : 0 ≤ s.re) :
  ...
  have hgamma : AnalyticAt ℂ (fun z : ℂ => Complex.Gamma (z / 2 + 1)) s := by
    ...
    convert (analyticAt_Gamma_of_re_pos ?_).comp_of_eq harg rfl using 1
    · ext z
      rfl
...
theorem eventually_riemannZetaDenom_ne_zero {s : ℂ} (hs : ∀ n : ℕ, s ≠ -2 * (n + 1)) :
    Filter.Eventually
      ...
      (nhds s) := by
  exact (continuousAt_riemannZetaDenom hs).eventually_ne (riemannZetaDenom_ne_zero hs)
...
theorem riemannXi_eventuallyEq_mul_riemannZeta {s : ℂ} (hs1 : s ≠ 1)
    ...
    Filter.EventuallyEq (nhds s) riemannXi
      (fun z : ℂ =>
        ((1 / 2 : ℂ) * (z - 1) * ((2 : ℂ) * (Real.pi : ℂ) ^ (-z / 2) * Complex.Gamma (z / 2 + 1))) *
          riemannZeta z) := by
  filter_upwards [eventually_ne_nhds hs1, eventually_riemannZetaDenom_ne_zero hs] with z hz1 hzD
  exact riemannXi_eq_mul_riemannZeta_of_denom_ne_zero hz1 hzD
...
theorem riemannXi_ne_zero_of_one_lt_re {s : ℂ} (hs : 1 < s.re) : riemannXi s ≠ 0 := by
  ...
  intro h
  exact riemannZeta_ne_zero_of_one_lt_re hs ((riemannXi_eq_zero_iff_of_denom_ne_zero hs1 hd).mp h)
  ...
  · have hs1 : s ≠ 1 := by
    ...
    exact
      RiemannZeta.riemannZeta_ne_zero_of_re_neg hs hnt
        ((riemannXi_eq_zero_iff_of_denom_ne_zero hs1 hd).mp h)
  · push Not at hnt
    obtain ⟨n, hn⟩ := hnt
