...
theorem quadraticCharacter_apply_odd_prime_natCast (n : ℕ) (hn : Odd n) {ℓ : ℕ} (hℓ : ℓ.Prime)
  ...
  simpa only [Int.cast_natCast] using
    quadraticCharacter_apply_odd_prime n hn hℓ hℓodd hcop
...
-/
theorem quadraticCharacter_sq (n : ℕ) (hn : Odd n) :
    quadraticCharacter n hn ^ 2 = 1 := by
...
theorem complexQuadraticCharacter_apply_odd_prime (n : ℕ) (hn : Odd n) {ℓ : ℕ} (hℓ : ℓ.Prime)
    (hℓodd : Odd ℓ) (hcop : Nat.Coprime ℓ n) :
    complexQuadraticCharacter n hn (ℓ : ZMod (4 * n)) =
      (jacobiSym n ℓ : ℂ) := by
...
theorem quadraticCharacter_ne_one_of_not_square (n : ℕ) (hn : Odd n) (hns : ¬IsSquare n) :
  ...
  · let u := ZMod.unitOfCoprime 5 (by norm_num only : Nat.Coprime 5 (4 * 3))
    ...
    have hvalue :=
      quadraticCharacter_apply_odd_prime_natCast 3 hn Nat.prime_five
        (by decide) (by norm_num only : Nat.Coprime 5 3)
    rw [hvalue]
  ...
  · obtain ⟨q, hqprime, hqodd, _, hqvalue⟩ :=
    ...
    have hcop : Nat.Coprime q n := by
      ...
      by_contra hgcd
      have hzero : jacobiSym n q = 0 :=
        jacobi_eq_zero_iff_not_coprime.mpr hgcd
      omega
    have hqne2 : q ≠ 2 := by
      intro hqtwo
      subst q
    ...
    have heq :=
      quadraticCharacter_apply_odd_prime_natCast n hn hqprime hqodd hcop
    rw [hqvalue] at heq
    rw [heq]
    norm_num only
...
theorem primeNegOneWitness_mem_of_complexQuadraticCharacter_ne_one (n : ℕ) (hn : Odd n) {ℓ : ℕ}
    (hℓprime : ℓ.Prime) (hℓndvd : ¬ℓ ∣ 4 * n)
    (hℓchar : complexQuadraticCharacter n hn ℓ ≠ 1) :
  ...
  have hvalue :=
    complexQuadraticCharacter_apply_odd_prime n hn hℓprime hℓodd hcop
