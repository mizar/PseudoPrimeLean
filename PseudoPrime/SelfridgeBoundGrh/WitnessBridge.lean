...
theorem primeNeOneWitness_le_classicalFirstStop_of_prime {n : ℕ} (hn : Odd n)
    ...
    {p : ℕ} (hp : p.Prime)
    (hstop :
      p = PrimeTest.firstStopNeOne PrimeTest.isClassicalCandidate n hs) :
    PrimeTest.primeNeOneWitness n hw ≤ p := by
  apply PrimeTest.primeNeOneWitness_le n hw
  subst p
  have hmem :=
    PrimeTest.firstStopNeOne_mem PrimeTest.isClassicalCandidate n hs
  refine ⟨hp, PrimeTest.isClassicalCandidate_odd hmem.1, ?_⟩
  rw [←
    PrimeTest.jacobi_selfridgeD (PrimeTest.isClassicalCandidate_odd hmem.1)
      hn]
  exact hmem.2.2
...
theorem primeNegOneWitness_le_classicalFirstStop_of_prime {n : ℕ} (hn : Odd n)
    ...
    {p : ℕ} (hp : p.Prime)
    (hstop :
      p = PrimeTest.firstStopNegOne PrimeTest.isClassicalCandidate n hs) :
    PrimeTest.primeNegOneWitness n hw ≤ p := by
  apply PrimeTest.primeNegOneWitness_le n hw
  subst p
  have hmem :=
    PrimeTest.firstStopNegOne_mem PrimeTest.isClassicalCandidate n hs
  refine ⟨hp, PrimeTest.isClassicalCandidate_odd hmem.1, ?_⟩
  rw [←
    PrimeTest.jacobi_selfridgeD (PrimeTest.isClassicalCandidate_odd hmem.1)
      hn]
