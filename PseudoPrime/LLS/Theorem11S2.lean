...
theorem not_characterTrivialBelow_log_sq {q : ℕ} [NeZero q] (χ : DirichletCharacter ℂ q)
    (hne : χ ≠ 1) (hq : 3000 ≤ q) (hGRH : AnalyticNumberTheory.GRH.GeneralizedRiemannHypothesis) :
  ...
  have hu' :=
    theorem11S2_logWeighted_le (S :=
      (AnalyticNumberTheory.Arithmetic.characterLogWeightedSum ((Real.log q) ^ 2)
          χ.primitiveCharacter).re)
      hy (hF.trans (sub_le_self _ (by norm_num only))) hb' (by nlinarith only [hu])
...
theorem exists_prime_not_one_le_log_sq {q : ℕ} [NeZero q] (χ : DirichletCharacter ℂ q) (hne : χ ≠ 1)
...
theorem llsTheorem11S2_of_grh (hGRH : AnalyticNumberTheory.GRH.GeneralizedRiemannHypothesis) :
    llsTheorem11S2 := by
  intro q _ hq _ H hH
  ...
  obtain ⟨p, hp, hb, hn⟩ :=
    exists_prime_not_one_le_log_sq (pullbackCharacter χ') (pullbackCharacter_ne_one_of_ne_one χ' hχ)
      hq hGRH
  refine ⟨p, hp, hb, ?_⟩
  rintro ⟨u, hu, heq⟩
  apply hn
