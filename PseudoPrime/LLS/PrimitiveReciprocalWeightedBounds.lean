...
theorem re_add_llsPrimitiveReciprocalResidues_zero_one_le {N : ℕ} [NeZero N] (hN2 : 2 ≤ N)
  ...
  · have hraw :=
      re_add_dirichletReciprocalResidues_zero_one_of_even_raw
    ...
    nlinarith [herr]
  · have hraw :=
      re_add_dirichletReciprocalResidues_zero_one_of_odd_raw
        hN2 hGRH hprimitive hne hinv hodd hxpos
...
theorem re_sum_llsPrimitiveReciprocalResidueAt_le {N : ℕ} [NeZero N] (hN2 : 2 ≤ N)
  ...
  have hxpos : (0 : ℝ) < x := by linarith
  rw [AnalyticNumberTheory.DirichletLFunction.dirichletSplitReciprocalSingularitySum x
      hne h1 h0]
  have h01 := re_add_llsPrimitiveReciprocalResidues_zero_one_le hN2 hGRH hprimitive hne hinv hx
  have hzeros :=
    AnalyticNumberTheory.DirichletLFunction.re_sum_erased_primitiveReciprocalResidues_le
...
theorem re_characterReciprocalWeightedSum_sub_leftVertical_le_of_grh {N : ℕ} [NeZero N]
  ...
  have htend :=
    tendsto_normalized_dirichletReciprocalBoundary_heightSeq_of_grh
  ...
  have hev :
    ...
    have hid :=
      dirichletReciprocalFiniteContourIdentity_heightSeq_normalized_of_grh
        hN2 hGRH hprimitive hne hinv (by linarith : (0 : ℝ) < x) A k hA
